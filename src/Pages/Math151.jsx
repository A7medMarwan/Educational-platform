...
export default function Math151() {
  ...
  const fetchUploadedFiles = async () => {
    ...
    // if (data) {
    //   setUploadedFiles(data.filter(f => f.name && f.name.trim() !== ""));
...
setUploadedFiles(
  ...
  const getPublicUrl = (fileName) => {
    return supabase.storage.from('uploads').getPublicUrl(`math151/${fileName}`).data.publicUrl;
  };
  ...
  const allFiles = [
    ...initialPdfFiles.map(f => ({
      ...f,
      isHardcoded: true,
      url: `/Assets/Materials/ريض 151/${encodeURIComponent(f.name.endsWith('.pdf') ? f.name : f.name + '.pdf')}`,
  ...
  return (
    <div style={{ background: "var(--secondary)", paddingBlock: "20px" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "2rem 1rem" }}>
        ...
        <FileUpload folderName="math151" onFilesAdded={handleUpload} />
